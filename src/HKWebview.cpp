						...
						[this](HRESULT result, ICoreWebView2Controller* controllerPtr) -> HRESULT
						{
							...
							*/
							...
							if (SUCCEEDED(webview.As(&webview3)))
							{
								...
								auto folder = exeDir / L"resources" / L"dist";

								OutputDebugStringW((L"EXE: " + exeDir.wstring() + L"\n").c_str());
								OutputDebugStringW((L"WEB: " + folder.wstring() + L"\n").c_str());

								if (!std::filesystem::exists(folder))
								{
									OutputDebugStringW(L"resources/dist NOT FOUND\n");
								}
